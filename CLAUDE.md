...
6. **Architecture & research:** `_docs/05-SITE-ARCHITECTURE.md`, `_docs/research/*.md`.
7. **Code:** `website/` is Next.js 16 (App Router, async `params`), React 19, Tailwind v4, TypeScript strict. All copy lives in `website/src/content/`. Read `website/node_modules/next/dist/docs/` before using unfamiliar Next APIs.
8. **Verify** with `npm run build` and `npm run lint` in `website/` before declaring anything done.
